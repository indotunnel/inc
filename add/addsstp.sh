...
sstp="$(cat /etc/accel-ppp.conf | grep port | sed s'/port=//g')"
until [[ $user =~ ^[a-zA-Z0-9_]+$ && ${CLIENT_EXISTS} == '0' ]]; do
		read -rp "Usernew : " -e user
		CLIENT_EXISTS=$(grep -w $user /var/lib/geovpn/data-user-sstp | wc -l)
...
$user * $pass *
EOF
echo -e "### $user $exp">>"/var/lib/geovpn/data-user-sstp"
clear
cat <<EOF
